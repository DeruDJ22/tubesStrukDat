...
void addEdge(graph &G, string startVertexID, string endVertexID, int weight) {
    if (weight < 0) {
        cout << "Jarak tidak dapat ditambahkan karena jaraknya negatif: " << weight << endl;
        return;
    } else if (startVertexID == endVertexID) {
        cout << "Kota tujuan tidak boleh sama dengan kota awal: " << endVertexID << endl;
        return;
    }
    ...
    if (start != NULL && end != NULL) {
        ...
        cout << "Salah satu atau kedua kota tidak ditemukan: " << startVertexID << " atau " << endVertexID << endl;
    }
}
